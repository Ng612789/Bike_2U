...
bool loadData() {
    ...
    // ========================================================
    // VALIDATE BICYCLE DATA
    // ========================================================

    bool validBicycleData =
        !bicycles.empty();
...
void memberTypeAnalytics() {
    ...
    for (const auto& c : customers) {
        ...
        if (count.find(c.memberType) != count.end()) {
            count[c.memberType]++;
        }
    }
    ...
    for (const string& type : types) {
        ...
        if (type == "Bronze")
            cout << "Basic membership\n";

        else if (type == "Silver")
            cout << "Regular membership\n";

        else
            cout << "Premium membership\n";
    }

    cout << string(70, '-') << '\n';

    cout << left
        << setw(18) << "Total Members"
        << setw(15) << total
        << "100.00%\n";
...
void bicycleAvailabilityAnalytics() {
    ...
    int grandRented =
        grandTotal - grandAvailable;

    double grandPercent =
        grandTotal == 0
        ? 0
        : grandAvailable * 100.0 / grandTotal;
    ...
    cout << "Available bicycles : "
        << grandAvailable
        << "\n";

    cout << "Currently rented   : "
        << grandRented
        << "\n";

    cout << "Bicycle availability : "
        << fixed
        << setprecision(1)
        << grandPercent
        << "%\n";
...
void revenueAnalytics() {
    ...
    cout << "| Average Transaction  | "
        << left
        << setw(20)
        << money(average)
        << "|\n";

    cout << "| Highest Transaction  | "
        << left
        << setw(20)
        << money(
            highest < 0
    ...
    for (const string& method :
        ...
        double methodRevenue =
            methods[method].second;
    ...
    if (!payments.empty()) {
        ...
        cout << "Best revenue month    : "
            << monthName(bestMonth)
            << " ("
            << money(monthly[bestMonth])
            << ")\n";

        cout << "Best revenue year     : "
            << bestYear
            << " ("
            << money(bestYearRevenue)
            << ")\n";

        cout << "Top payment method    : "
            << bestMethod
            << " ("
            << money(bestMethodRevenue)
            << ")\n";

        cout << "Highest payment       : "
            << money(highest)
            << " by "
            << highestCustomer
            << " (Payment #"
...
void bicycleAnalysis() {
    ...
    if (bicycles.size() > 0) {
        ...
        cout << "Largest bicycle category : "
            << mostCommon
            << " ("
            << counts[mostCommon]
            << " bicycles)\n";

        cout << "Smallest bicycle category: "
            << leastCommon
            << " ("
            << counts[leastCommon]
            << " bicycles)\n";
...
int main() {
    ...
    // Load existing data from admin.csv
    loadData();
