...
void adminMenu(vector<RepairReport>& repairs, vector<Account>& users) {
    ...
    while (true) {
        ...
        cout << indent << "[1] Account Management\n";   // 改为 Account Management
        cout << indent << "[2] Repair Service\n";
        cout << indent << "[3] Logout\n";
...
int registerUser(vector<Account>& users) {
    ...
    if (trim(confirmPassword) == "b" || trim(confirmPassword) == "B")
        return -2;
...
void deleteAcc(vector<RepairReport>& repairs, vector<Account>& users) {
    ...
    cout << indent << "Enter the Email or Account ID of the user to delete: ";
    string input;
    getline(cin, input);
    if (input.empty()) {
    ...
    cout << indent << "Are you sure you want to delete this account? (Y/N): ";
    string confirm;
    getline(cin, confirm);
    if (confirm != "Y" && confirm != "y") {
