...
string getStationFromUser();  // NEW helper

// Rental booking functions
void rentalBookingMenu(vector<Booking>& bookings, vector<Bicycle>& inventory, const string& customerID, const string& customerName, const vector<RepairReport>& repairs);
void CreateBooking(vector<Booking>& bookings, vector<Bicycle>& inventory, const string& customerID, const string& customerName, const vector<RepairReport>& repairs);
void ViewActiveBookings(const vector<Booking>& bookings);
void ReturnBicycle(vector<Booking>& bookings, vector<Bicycle>& inventory, const string& customerID);
...
void userMenu(vector<Account>& users, int currentIdx, vector<RepairReport>& repairs, vector<Bicycle>& inventory, vector<Booking>& bookings) {
    ...
    while (true) {
        ...
        switch (option) {
        case 1:
            rentalBookingMenu(bookings, inventory, users[currentIdx].accountID, users[currentIdx].name, repairs);
            break;
        case 2:
...
int registerUser(vector<Account>& users) {
    ...
    cout << et << "Email (e.g user@example.com)" << string(3, ' ') << ": ";
...
void deleteAcc(vector<RepairReport>& repairs, vector<Account>& users) {
    ...
    if (users.empty()) {
        cout << indent << "No users to delete.\n";
        waitForEnter(indent);
        return;
    }

    cout << "\n" << indent << "--- All Registered Users ---\n";
    cout << left << setw(5) << "No." << setw(12) << "Account ID" << setw(20) << "Name" << setw(25) << "Email" << setw(15) << "Phone" << "\n";
    cout << string(77, '-') << "\n";
    for (size_t i = 0; i < users.size(); ++i) {
        const Account& user = users[i];
        cout << left << setw(5) << (i + 1) << setw(12) << user.accountID << setw(20) << user.name.substr(0, 19) << setw(25) << user.email.substr(0, 24) << setw(15) << user.phone << "\n";
    }
    cout << string(77, '-') << "\n\n";
...
string getStationFromUser() {
    while (true) {
        cout << "\n--- Available Stations (Penang) ---\n";
        for (size_t i = 0; i < PENANG_STATIONS.size(); ++i) {
            cout << "  [" << (i + 1) << "] " << PENANG_STATIONS[i] << "\n";
        }
        ...
        getline(cin, input);
        if (input.empty()) {
            cout << "[X] Please enter a number between 1 and " << PENANG_STATIONS.size() << ".\n";
            continue;
        }
        bool isNumber = true;
        for (char c : input) {
    ...
    while (true) {
        ...
        case 1:
            ...
            if (inventory[idx].type.empty()) {
                cout << "[X] Type cannot be empty. Keeping old value.\n";
                waitForEnter();
            }
            else {
        ...
        case 2:
            ...
            if (inventory[idx].brand.empty()) {
                cout << "[X] Brand cannot be empty. Keeping old value.\n";
                waitForEnter();
            }
            else {
                cout << "Brand updated.\n";
...
void rentalBookingMenu(vector<Booking>& bookings, vector<Bicycle>& inventory, const string& customerID, const string& customerName, const vector<RepairReport>& repairs) {
    while (true) {
        ...
        switch (option) {
        case 1:
            CreateBooking(bookings, inventory, customerID, customerName, repairs);
            break;
        case 2:
...
void CreateBooking(vector<Booking>& bookings, vector<Bicycle>& inventory, const string& customerID, const string& customerName, const vector<RepairReport>& repairs) {
    ...
    for (const auto& r : repairs) {
        if (r.bicycleID == bikeID && r.status == "Pending") {
            hasPendingRepair = true;
            break;
        }
    }
    if (hasPendingRepair) {
        cout << "Error: This bicycle" << bikeID << "is connot available and cannot be rented.\n";
        waitForEnter();
        return;
    }
    ...
    if (baseRate <= 0) {
        cout << "[X] Invalid bike rate. Booking cancelled.\n";
        waitForEnter();
        return;
    }
    ...
    bool paid = ProcessPaymentForBooking(tempBooking, duration, baseRate, customerName);
    ...
    Booking newBooking = tempBooking;
...
void ViewActiveBookings(const vector<Booking>& bookings) {
    ...
    cout << "\n" << left << setw(12) << "BookingID" << setw(12) << "CustomerID" << setw(12) << "BikeID" << setw(10) << "Duration" << setw(12) << "Status" << endl;
    cout << string(58, '-') << endl;

    for (const auto& b : bookings) {
        if (b.status == "Active") {
            cout << left << setw(12) << b.bookingID << setw(12) << b.customerID << setw(12) << b.bikeID << setw(10) << (to_string(b.duration) + " hrs")
...
void ReturnBicycle(vector<Booking>& bookings, vector<Bicycle>& inventory, const string& customerID) {
    ...
    for (auto& booking : bookings) {
        if (booking.bookingID == bookingID && booking.status == "Active") {
            ...
            SyncInventoryWithBookings(bookings, inventory); // 同步
            SaveInventoryToFile(inventory);                 // 也要save进Inventory
            SaveBookingsToFile(bookings, BOOKING_FILE);
...
void SaveBookingsToFile(const vector<Booking>& bookings, const string& filename) {
    ...
    for (const auto& b : bookings) {
        outFile << b.bookingID << "," << b.customerID << "," << b.bikeID << "," << b.duration << "," << b.status << "\n";
    }
    outFile.close();
}
    ...
    if (methodChoice == 1 || methodChoice == 2) {
        ...
        do {
            ...
            if (!isValidCardNumber(cardNum)) {
                cout << "[X] Invalid card number.\n";
                continue;
            }
            cout << "Expiry (MM/YY): ";
            getline(cin, expiry);
            if (expiry.length() != 5 || expiry[2] != '/') {
                cout << "[X] Invalid expiry format.\n";
                continue;
            }
            cout << "CVV (3 digits): ";
            getline(cin, cvv);
...
void requestRefund(vector<Account>& users, int currentIdx, vector<Booking>& bookings, vector<RepairReport>& repairs, vector<Payment>& transactions) {
    ...
    for (size_t i = 0; i < userBookings.size(); ++i) {
        cout << indent << "[" << i + 1 << "] Booking " << userBookings[i].bookingID << " | Bike: " << userBookings[i].bikeID
            << " | Duration: " << userBookings[i].duration << "h" << " | Status: " << userBookings[i].status << "\n";
    }
