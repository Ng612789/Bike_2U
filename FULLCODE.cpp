...
void ViewMyBookings(const vector<Booking>& bookings, const string& customerID) {
    ...
    int page = 0;
    int totalPages = static_cast<int>((myBookings.size() + PER_PAGE - 1) / PER_PAGE);
...
void DisplayUserPaymentHistory(const string& customerName, const vector<Booking>& bookings, const vector<Bicycle>& inventory) {
    ...
    const int PER_PAGE = 5;
    int page = 0;
    int totalPages = static_cast<int>((customerPayments.size() + PER_PAGE - 1) / PER_PAGE);
...
void reportingMenu(vector<Account>& users, vector<Bicycle>& inventory, vector<Payment>& transactions) {
    while (true) {
        ...
        switch (op) {
        case 1: memberTypeAnalytics(users);
            break;
        case 2: bicycleAvailabilityAnalytics(inventory);
            break;
        case 3: revenueAnalytics(transactions);
            break;
        case 4: bicycleAnalysis(inventory);
            break;
        case 0:
            if (confirmAction("Are you sure want to return back?")) {
                cout << "Returning...\n";
